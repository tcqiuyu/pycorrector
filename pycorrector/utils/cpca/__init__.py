...
def _data_from_csv() -> (AddrMap, AddrMap, AddrMap, dict, dict):
    ...
    with resource_stream('pycorrector.resources', 'pca.csv') as pca_stream:
...
def _fill_province(pca):
    ...
    if (not pca.province) and pca.city and (pca.city in city_map):
        ...
        return
    if (not pca.province) and pca.area and len(area_map.get_relational_addrs(pca.area)) == 1:
        # 根据区填充（若区只对应一个省）
        pca.province = area_map.get_relational_addrs(pca.area)[0][0]
        return
...
def _full_text_extract(addr, lookahead):
    ...
    while i <= max_len:
        ...
        for l in range(lookahead, 0, -1):
            word = addr[i:i + l]
            # 优先提取低级别的行政区 (主要是为直辖市和特别行政区考虑)
            if word in area_map and _to_filled_is_valid(result, word, "area"):
                defer_fun = _set_pca('area', i, word, area_map.get_full_name(word))
                defer_fun()
                _fill_city(result, myumap)
                _fill_province(result)
                break
                # continue
            elif word in city_map and _to_filled_is_valid(result, word, "city"):
                defer_fun = _set_pca('city', i, word, city_map.get_full_name(word))
                defer_fun()
                _fill_province(result)
                break
                # continue
            elif word in province_map and _to_filled_is_valid(result, word, "province"):
...
def _check_finished(pca: Pca):
    # if pca.
    pass


def _to_filled_is_valid(pca: Pca, to_filled: str, pos: str):
    """
    检查添加到PCA的部分是否与现有信息矛盾，若矛盾则返回False
    :param pca: 已建立的PCA对象
    :param to_filled: 添加的word
    :param pos: 添加词所属的pos: province/city/area (省/市/区)
    :return:
    """
    if pos == "province":
        # 经查，没有相同市不同省
        province = province_map.get(to_filled)
        all_possible_province_city_map = province_area_map.get_relational_addrs((province, ""))
        all_possible_cities = [res[1] for res in all_possible_province_city_map]
        all_possible_areas = []
        for city in all_possible_cities:
            for tup in city_map.get_relational_addrs(city):
                all_possible_areas.append(tup[2])
        if pca.city != "" and pca.city not in all_possible_cities:
            # 存在市且市与填入省可能包含市矛盾
            return False
        if pca.area != "" and pca.area not in all_possible_areas:
            # 存在区且区与填入省可能包含区矛盾
            return False
        return True
    elif pos == "city":
        all_possible_results = city_map.get_relational_addrs(to_filled)
        all_possible_provinces = [res[0] for res in all_possible_results]
        all_possible_areas = [res[2] for res in all_possible_results]
        if pca.province != "" and pca.province not in all_possible_provinces:
            # 存在省份且省份与填入市可能所属省份矛盾
            return False
        if pca.area != "" and pca.area not in all_possible_areas:
            # 存在区且区与填入市可能包含区矛盾
        ...
        all_possible_results = area_map.get_relational_addrs(to_filled)
        all_possible_provinces = [res[0] for res in all_possible_results]
        all_possible_cities = [res[1] for res in all_possible_results]
        if pca.province != "" and pca.province not in all_possible_provinces:
            # 存在省份且省份与填入区可能所属省份矛盾
            return False
        if pca.city != "" and pca.city not in all_possible_cities:
